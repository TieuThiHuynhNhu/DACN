using DACN.Libraries;
using DACN.Models.VNPAY;

namespace DACN.Services.VNPay
{
    public class VnPayService : IVnPayService
    {
        ...
        public string CreatePaymentUrl(PaymentInformationModel model, HttpContext context)
        {
            ...
            // 🔹 Quan trọng: mã đặt phòng được truyền về callback để xác định đơn hàng
        ...
        public PaymentResponseModel PaymentExecute(IQueryCollection collections)
        {
            ...
            {
                var pay = new VnPayLibrary();
                result = pay.GetFullResponseData(collections, _configuration["Vnpay:HashSecret"]);
                return result;
            }
            catch (Exception ex)
            {
